...
def linear_estimated_classifiers(contexts, classifiers, costs, algorithm_parameters):
    ...
    for t in range(m, data_points):
        ...
        for a in range(K):
            ...
            # beta_t = R * np.sqrt(d * np.log(1 + ((arm_counter[a] * L) / (lambda_val * d))) +
            #                      2 * np.log(1 / delta) + np.sqrt(lambda_val) * S)
...
def regret_plotting(regret, classifier_count):
    ...
    plt.xlabel("Horizon")
    plt.savefig("output/plots/regret_k3_" + str(data_points) + ".png", bbox_inches='tight')
    plt.close()


# ####################### Reading given data #########################
# Classifiers and costs
synthetic_classifiers = np.array([[[1, 1, 0], [2, -1, 1]],
                                  [[1, 1, 0], [-2, -1, 1], [2, -1, 1]],
                                  [[1, 1, 1.4], [1, 1, 0.6], [1, 1, -0.1], [1, 1, -8]],
                                  [[1, 1, 1.4], [1, 1, 0.6], [1, 1, -0.1], [1, 1, -8], [1, 1, -1.6]],
                                  [[1, 1, 1.8], [1, 1, 1.4], [1, 1, 1], [1, 1, 0.6], [1, 1, 0.2],
                                   [1, 1, -0.2], [1, 1, -0.6], [1, 1, -1], [1, 1, -1.4], [1, 1, -1.8]]
                                  ])
classifier_costs = [[0.2, 0.7],
                    [0.14, 0.43, 0.81],
                    [0.16, 0.30, 0.54, 0.9],
                    [0.12, 0.28, 0.45, 0.68, 0.95],
                    [0.03, 0.08, 0.14, 0.21, 0.30, 0.4, 0.52, 0.68, 0.81, 0.98]]

# Algorithm parameters: [alpha, lambda_val, K, d, m, R, delta, S, L]
parameters = [1, 0.5, 2, 3, 1, 0.01, 0.1, 5.24, 3]
...
classifier_regret = []
for c in range(len(synthetic_classifiers)):
    file_name = open("output/regretFiles/2d/" + str(data_points) + "_" + str(classifiers_count[c]) + ".txt", 'w')
