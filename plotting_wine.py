...
def regret_plotting(regret, classifier_count, data_points):
    ...
    # plt.title("Cumulative Regret for different numbers of classifiers for Wine dataset")
    plt.ylabel("Cumulative Regret")
    plt.xlabel("Number of Samples")
    plt.savefig("output/final_plot/wine_" + str(data_points) + ".png", bbox_inches='tight')
    plt.close()


# Reading Data
data_plot = [4898]
# classifiers_count = [2, 3, 4, 5]
...
for d in range(len(data_plot)):
    ...
    for c in classifiers_count:
        fileName = "output/regretFiles/wine/" + str(data_plot[d]) + "_" + str(c) + ".txt"
