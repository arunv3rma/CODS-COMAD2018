...
def regret_plotting(regret, classifier_count, data_points):
    ...
    # plt.title("Cumulative Regret for different numbers of classifiers for News Dataset")
    plt.ylabel("Cumulative Regret")
    plt.xlabel("Number of Samples")
    plt.savefig("output/final_plot/news_" + str(data_points) + ".png", bbox_inches='tight')
    plt.close()
...
for d in range(len(data_plot)):
    ...
    for c in classifiers_count:
        fileName = "output/regretFiles/news/" + str(data_plot[d]) + "_" + str(c) + ".txt"
